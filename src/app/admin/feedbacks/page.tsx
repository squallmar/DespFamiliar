...
export default function AdminFeedbacksPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/feedbacks")
      ...
      .catch(() => {
        setError(tr('errorLoadingFeedbacks', 'Erro ao carregar feedbacks'));
        setLoading(false);
      });
  }, [user]);

  const pageMap: Record<string, string> = {
    '/profile': tr('pageProfile', 'Perfil'),
    '/': tr('pageDashboard', 'Dashboard'),
    '/expenses': tr('pageExpenses', 'Despesas'),
    '/projections': tr('pageProjections', 'Projeções'),
    '/reports': tr('pageReports', 'Relatórios'),
    '/login': tr('pageLogin', 'Login'),
    '/register': tr('pageRegister', 'Cadastro'),
    '/reset': tr('pageReset', 'Recuperação de senha'),
    '/admin/users': tr('pageAdminUsers', 'Admin: Usuários'),
    '/admin/feedbacks': tr('pageAdminFeedbacks', 'Admin: Feedbacks'),
  };
  ...
  const allPagesLabel = useMemo(() => {
    return tr('all', 'Todas as páginas');
  }, [t]);
  ...
  if (!user?.admin) {
    return <div className="p-8 text-red-600 font-bold">{tr('adminAccessRestricted', 'Acesso restrito ao administrador.')}</div>;
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <h1 className="text-2xl font-bold mb-6 flex items-center gap-2">
        <span className="inline-block bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-base font-semibold">{tr('feedbacksReceived', 'Feedbacks recebidos')}</span>
        <span className="ml-2 text-gray-400 text-sm font-normal">({filteredFeedbacks.length}/{feedbacks.length})</span>
      </h1>
      {loading ? (
        <div>{tr('loading', 'Carregando...')}</div>
      ) : error ? (
        <div className="text-red-600">{tr('error', error)}</div>
      ) : feedbacks.length === 0 ? (
        <div className="text-gray-500">{tr('noFeedbacksYet', 'Nenhum feedback recebido ainda.')}</div>
      ) : (
        <div className="space-y-4">
          <div className="bg-white rounded-lg shadow p-4 border border-gray-100 flex flex-col md:flex-row gap-3">
            <input
              ...
              placeholder={tr('search', 'Buscar por mensagem, email ou página...')}
            ...
            >
              <option value="all">{allPagesLabel}</option>
          ...
          {filteredFeedbacks.length === 0 ? (
            <div className="text-gray-500 bg-white rounded-lg shadow p-6 border border-gray-100">
              {tr('noResults', 'Nenhum feedback encontrado com os filtros atuais.')}
            </div>
            ...
            <div key={fb.id} className="bg-white rounded-lg shadow p-4 border border-gray-100">
              ...
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                {fb.email && (
                  ...
                  >
                    {copiedKey === `${fb.id}:email` ? tr('copied', 'Copiado!') : tr('copyEmail', 'Copiar email')}
                  </button>
                )}
                <button
                ...
                >
                  {copiedKey === `${fb.id}:message` ? tr('copied', 'Copiado!') : tr('copyMessage', 'Copiar mensagem')}
                </button>
                <button
                ...
                >
                  {copiedKey === `${fb.id}:all` ? tr('copied', 'Copiado!') : tr('copyAll', 'Copiar tudo')}
                </button>
              </div>
                );
              })()}
