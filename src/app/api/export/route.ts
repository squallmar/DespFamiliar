...
import PDFDocument from 'pdfkit';
    if (type === 'pdf') {
      ...
      expenses.forEach((e: any) => {
        doc.text(new Date(e.date).toLocaleDateString('pt-BR'), 30, doc.y, { continued: true, width: 70 });
        doc.text(Number(e.amount).toFixed(2), 100, doc.y, { continued: true, width: 60 });
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const e of expenses) {
      rows.push([
        escape(e.id),
        escape(new Date(e.date).toISOString()),
        escape(e.amount),
