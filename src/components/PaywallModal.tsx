...
const PaywallModal: React.FC<PaywallModalProps> = ({ open, onClose, onUpgrade }) => {
  ...
  const handlePix = () => {
    window.open('https://api.whatsapp.com/send?phone=5509860867704&text=Quero%20assinar%20o%20premium%20por%20R$20%20via%20Pix', '_blank');
  };
              ...
              <div className="space-y-3">
                {!loading && countryCode === 'BR' ? (
                  ...
                  >
                    ...
                    {t('subscribePix') || 'Assinar via Pix'}
                  </button>
                ) : !loading && (
                  <>
                    <button
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-gray-900 text-white rounded-lg font-bold text-lg hover:bg-gray-800 transition shadow-lg hover:shadow-xl cursor-pointer"
                    >
                      {t('subscribeCrypto') || 'Pagar com Cripto'}
                    </button>
                  </>
                )}
              </div>
